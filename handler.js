...
//const isPrems = isROwner || global.prems.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
        ...
        // Función para normalizar cualquier tipo de ID (@lid, @s.whatsapp.net, etc)
        const normalizeId = (jid) => {
            ...
            // Si es un @lid mapeado, usa el ID real
            if (decoded in LID_TO_NUMBER) {
                return LID_TO_NUMBER[decoded].split('@')[0];
            }
            // Si no, extrae solo la parte numérica
            return decoded.endsWith('@lid') ? decoded.split('@')[0] : decoded.replace(/@.+/, '');
        };
...
// Normalizar IDs para comparación
        const senderNormalized = normalizeId(m.sender);
        const botNormalized = normalizeId(this.user.jid);

// Buscar usuario y bot
        const user = m.isGroup
            ? participants.find(u => normalizeId(u.id) === senderNormalized)
            : {};
        const bot = m.isGroup
            ? participants.find(u => normalizeId(u.id) === botNormalized)
            : {};

// Verificar roles
        const isRAdmin = user?.admin === 'superadmin' || false;
        const isAdmin = isRAdmin || (user?.admin && ['admin', 'superadmin'].includes(user.admin)) || false;
...
export async function participantsUpdate({ id, participants, action }) {
    ...
    let text = '';
    let imageToSend;  // Default image is imagen16
            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        // Si no puede obtener la foto de perfil, no hace nada
                    } finally {
                        const botTt2 = groupMetadata.participants.find(u => this.decodeJid(u.id) == this.user.jid) || {};
                        const isBotAdminNn = botTt2?.admin === "admin" || false;

                        // Si la acción es 'add' (usuario se añade)
                        // Si la acción es 'add' (usuario se añade)
                        if (action === 'add') {
                            console.log("id" + id)
                            const welcomeImagePath = `./imagesGroups/${id.replace('@g.us', '')}welcome.jpg`;
                            let imageB;

                            if (fs.existsSync(welcomeImagePath)) {
                                console.log(`✅ Imagen personalizada encontrada: ${welcomeImagePath}`);
                                imageB = welcomeImagePath;
                            } else {
                                console.log(`⚠️ No se encontró imagen personalizada, usando imagen por defecto`);
                                imageB = imagen16;
                            }

                            const outputPath = './media/output.jpg';
                            imageToSend = imagen16;

                            try {
                                // Obtener imagen de perfil con axios
                                let perfilBuffer;
                                try {
                                    const response = await axios.get(pp, { responseType: 'arraybuffer' });
                                    perfilBuffer = Buffer.from(response.data, 'binary');
                                } catch (e) {
                                    perfilBuffer = imagen20;
                                }

                                // 1️⃣ Obtener tamaño de la imagen base
                                const baseMeta = await sharp(imageB).metadata();
                                const baseWidth = baseMeta.width;
                                const baseHeight = baseMeta.height;

                                // 2️⃣ Tamaño del círculo como % del ancho de la base
                                const circleSize = Math.floor(baseWidth * 0.25); // 25% del ancho
                                const circleRadius = Math.floor(circleSize / 2);

                                // 3️⃣ Crear máscara circular proporcional
                                const mask = await sharp({
                                    ...
                                    .composite([{
                                        input: Buffer.from(
                                            `<svg><circle cx="${circleRadius}" cy="${circleRadius}" r="${circleRadius}" fill="white"/></svg>`
                                        ),
                                        top: 0,
                                        left: 0
                                    }])
                                ...
                                // 4️⃣ Recortar perfil en círculo proporcional
                                const perfilRedondo = await sharp(perfilBuffer)
                                    .resize(circleSize, circleSize)
                                    .composite([{ input: mask, blend: 'dest-in' }])
                                    .png()
                                    .toBuffer();

                                // 5️⃣ Texto proporcional al tamaño base
                                const fontSize = Math.floor(baseWidth * 0.04); // 8% del ancho
                                const nombre = `Holiss ❤`;
                                const svgText = `
<svg width="${baseWidth}" height="${fontSize + 50}">
    <style>
        .title { fill: #9B30FF; font-size: ${fontSize}px; font-weight: bold; font-family: sans-serif; text-anchor: middle; }
    </style>
    <text x="${baseWidth / 2}" y="${fontSize}" class="title">${nombre}</text>
</svg>
        `;
                                const textoBuffer = Buffer.from(svgText);

                                // 6️⃣ Unir perfil y texto en bloque seguro
                                const bloqueWidth = Math.max(circleSize, baseWidth);
                                const bloqueHeight = circleSize + fontSize + 20;

                                const perfilConTexto = await sharp({
                                    create: {
                                        width: bloqueWidth,
                                        height: bloqueHeight,
                                        channels: 4,
                                    ...
                                    .composite([
                                        {
                                            input: perfilRedondo,
                                            top: 0,
                                            left: Math.floor((bloqueWidth - circleSize) / 2) // centrado en bloque
                                        },
                                        {
                                            input: textoBuffer,
                                            top: circleSize,
                                ...
                                // 7️⃣ Calcular coordenadas para centrar en la imagen base
                                const perfilX = Math.floor((baseWidth - bloqueWidth) / 2);
                                const perfilY = Math.floor((baseHeight - bloqueHeight) * 0.90);

                                // 8️⃣ Combinar con la imagen base
                                await sharp(imageB)
                                    .composite([{ input: perfilConTexto, top: perfilY, left: perfilX }])
                                    .toFile(outputPath);

                                imageToSend = outputPath;
                        ...
                        // Si la acción es 'remove' (usuario se elimina)
                            ...
                            console.log("id" + id)
                            const welcomeImagePath = `./imagesGroups/${id.replace('@g.us', '')}bye.jpg`;
                            let imageB;
    ...
    <text x="${baseWidth / 2}" y="${fontSize}" class="title">${nombre}</text>
        ...
        `;
                        ...
                        if (chat.antifake && isBotAdminNn && action === 'add') {
                            const numerosPermitidos = ["1", "2", "4", "6", "7", "8", "9"];  // Puedes editar los números
                            if (numerosPermitidos.some(num => user.startsWith(num))) {
                                this.sendMessage(id, {
                                    text: `*${lenguajeGB['smsAvisoAG']()}${lenguajeGB['smsInt1']()} @${user.split("@")[0]} ${lenguajeGB['smsInt2']()}*`,
                                    mentions: [user]
                                }, { quoted: null });
                        ...
                        // Enviar la imagen y mensaje adecuado dependiendo de la acción
                        let fkontak2 = {
                        ...
                        // Enviar la imagen adecuada
                        await this.sendFile(id, imageToSend, 'pp.jpg', text, null, false, { mentions: [user] }, { quoted: fkontak2 });
...
export async function deleteUpdate(message) {
    try {
        ...
        const antideleteMessage = `${lenguajeGB['smsCont21']()}@${participant.split`@`[0]}`.trim();
...
//if (msg) return m.reply(msg)
    ...
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: lenguajeGB.smsAvisoAG().slice(0,-2), body: [wm, '🐈‍⬛ 𝗦𝘂𝗽𝗲𝗿 ' + gt + ' 🐈‍⬛', '❤ arletsitabot.gmail.com'].getRandom(), thumbnail: gataImg.getRandom(), sourceUrl: [md, channel1, channel2, channel3, yt, ig, paypal, fb].getRandom() }}}}, tg)
