...
if (!db.data.chats[m.chat].modohorny && m.isGroup) throw `[ ⚠️ ] Los comandos +18 estan desactivados en este grupo, si es administrador de este grupo y desea activarlos escriba #enable modohorny para activar*`
 ...
 const ahora = new Date();
    const horaMX = new Date(ahora.toLocaleString("en-US", {timeZone: "America/Mexico_City"}));
    const horas = horaMX.getHours();
    
    // Validar horario: 10 PM (22) a 4 AM (4)
    if (horas < 22 && horas >= 4) {
        await new Promise(resolve => setTimeout(resolve, 1000));
    
    // URL del video
    const videoUrl = 'https://files.catbox.moe/x0y8p2.mp4';
    ...
    // Crear el sticker con el Buffer
    let stiker = await sticker(videoBuffer, false, global.packname, global.author);
    
    // Enviar el sticker
    await conn.sendFile(m.chat, stiker, 'sticker.webp', null, m, false);
      return await m.reply(`*⏰ Este comando solo está disponible de 10 PM a 4 AM*\n*Hora actual: ${horas}:${horaMX.getMinutes().toString().padStart(2, '0')}*`);
    }
    
    
  switch (command) {
    case 'pack': case 'packgirl':
          try{
    await m.react('🔥');
    ...
    await conn.sendMessage(
        ...
        { quoted: m }
      		...
      		conn.sendMessage(m.chat, {image: {url: url}, caption: `🥵`,viewOnce: true}, {quoted: m});
          }
      break;
    case 'packgirl':
       try{
    ...
    case 'packmen':
      ...
      conn.sendMessage(m.chat, {image: {url: url3}, caption: `🥵`,viewOnce: true}, {quoted: m});
      break;
    case 'vidx':
      const url4 = await videosxxxc[Math.floor(Math.random() * videosxxxc.length)];
      await conn.sendMessage(m.chat, {video: {url: url4}, caption: `*🥵*`,viewOnce: true}, {quoted: m});
      break;
    case 'vidxlesbi':
...
handler.command = /^(pack|packgirl|packmen|vidx|vidxlesbi)$/i;
