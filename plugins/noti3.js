...
const notifitimePath = '/home/container/src/game/notifitime.json';
...
const handler = async (m, { conn, text, participants, isOwner, isAdmin, args, command, owner }) => {
    ...
    if (command == '6' || command == 6) {
        ...
        messageToSend = `*+4*  ${messageToSend}\n                                                     ᴬʳˡᵉᵗᵗᴮᵒᵗ`;
    }else if(command == '16' || command == 16){
        messageToSend = `*+16*  ${messageToSend}\n                                                     ᴬʳˡᵉᵗᵗᴮᵒᵗ`;
    }else if(command == '5' || command == 5){
        messageToSend = `*+5*  ${messageToSend}\n                                                     ᴬʳˡᵉᵗᵗᴮᵒᵗ`;
    }else if(command == '15' || command == 15){
        messageToSend = `*+15*  ${messageToSend}\n                                                     ᴬʳˡᵉᵗᵗᴮᵒᵗ`;
    }else if(command == '3' || command == 15){
        messageToSend = `*+3*  ${messageToSend}\n                                                     ᴬʳˡᵉᵗᵗᴮᵒᵗ`;
    }
...
handler.command = /^(notify3|noti3|6|1|2|12|4|5|16|15|3)$/i;
handler.group = true;
