...
const smsOwner = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO MI CREADORA LO PUEDE USAR!!\`\`\`` }//OWNER
...
//main.js
const smsWelcome = () => { return `🌸 ¡Holis <3, @user!`}
const smsBye = () => { return '😘 Se fue @user\n' +
    'Una lástima... justo cuando estaba aprendiendo tu nombre 🖤'}
const smsSpromote = () => { return '*@user 𝙀𝙎 𝘼𝘿𝙈𝙄𝙉!!*'}
const smsSdemote = () => { return '*@user 𝘿𝙀𝙅𝘼 𝘿𝙀 𝙎𝙀𝙍 𝘼𝘿𝙈𝙄𝙉!!*'}
...
//_allantilink.js
const smsTextoYT = () => { return ' 🐈‍⬛  𝑨𝒓𝒍𝒆𝒕𝒔𝒊𝒕𝒂 𝑩𝒐𝒕 ❤️ '} 
const smsApagar = () => { return ' *OFF* ❌'} 
const smsEncender = () => { return ' *ON* ✅'} 
...
const smsCont5 = () => { return `*❌*`}
const smsCont6 = () => { return `*❗ 𝗥𝗘𝗣𝗢𝗥𝗧𝗘 𝗘𝗦𝗧𝗘 𝗠𝗘𝗡𝗦𝗔𝗝𝗘 𝗨𝗦𝗔𝗡𝗗𝗢 𝗘𝗟 𝗖𝗢𝗠𝗔𝗡𝗗𝗢 #reporte 𝗣𝗔𝗥𝗔 𝗦𝗢𝗟𝗨𝗖𝗜𝗢𝗡𝗔𝗥𝗟𝗢*`}
const smsCont7 = () => { return `${global.lenguajeGB['smsAvisoAG']()}*NO TIENE DIAMANTES!! 💎 PUEDE JUGAR TRIVIA O IR A LA TIENDA CON EL COMANDO*`}
...
const smsCont21 = () => { return `*Enviando el mensaje eliminado de:*\n  `}
...
const smskick2 = () => { return `-1 Put@ 👻`}
const smskick3 = () => { return `ES EL CREADOR DEL GRUPO`}
