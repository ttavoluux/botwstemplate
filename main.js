...
console.log('Valor de methodCodeQR:', methodCodeQR);
...
//if (MethodMobile) throw new Error('No se puede usar un código de emparejamiento con la API móvil')
            ...
            let addNumber
            phoneNumber='52' + getOwner()



            setTimeout(async () => {
                const numeroMexico = phoneNumber;
                let codeBot = await conn.requestPairingCode(numeroMexico)
...
if (!opts['test']) {
    if (global.db) setInterval(async () => {
        ...
        if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp'], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
function purgeOldFiles() {
    const directories = ['./GataBotSession/']
...
setInterval(async () => {
    ...
    await clearTmp()
    console.log(chalk.bold.cyanBright(lenguajeGB.smsClearTmp()))}, 1000 * 60 * 10) // 4 min
